...
class TicketController extends Controller
{
    ...
    public function show(Ticket $ticket)
    {
        ...
        // Prepare QR code for display
        $qrBase64 = null; // On initialise la variable

        if ($ticket->qr_code) { // Si un chemin de QR code existe pour ce ticket
            $filePath = public_path($ticket->qr_code); // On construit le chemin complet vers le fichier public

            if (File::exists($filePath)) { // Si le fichier existe réellement
                try {
                    $fileContent = File::get($filePath); // On lit le contenu du fichier
                    // Comme vos QR codes sont des SVG :
                    $mimeType = 'image/svg+xml';
                    // On encode en base64 et on préfixe avec le type MIME pour l'utilisation dans src=""
                    $qrBase64 = 'data:' . $mimeType . ';base64,' . base64_encode($fileContent);
                } catch (\Exception $e) {
                    Log::error("Failed to read or encode QR code file for ticket {$ticket->id} at path {$filePath}: " . $e->getMessage());
                    // $qrBase64 reste null, la vue affichera "QR Code non disponible."
                }
            } else {
                Log::warning("QR code file not found for ticket {$ticket->id} at path: {$filePath}");
                // $qrBase64 reste null, la vue affichera "QR Code non disponible."
            }
        } else {
            Log::info("Ticket {$ticket->id} does not have a qr_code path specified.");
            // $qrBase64 reste null, la vue affichera "QR Code non disponible."
        }
        // --- FIN DU CODE AJOUTÉ ---

        // Store the current ticket ID in the session to preserve it
        // Use put instead of flash to keep the value across requests
        session()->put('viewed_ticket_id', $ticket->id);

        // Note: We don't need to do anything with created_tickets
        // since we're using session()->put instead of flash,
        // existing session values will be maintained

        // Pass the ticket object AND the qrBase64 string to the view
        return view('tickets.show', compact('ticket', 'qrBase64')); // <--- MAINTENANT $ticket ET $qrBase64 SONT PASSÉS
    }
    public function index(Request $request)
    {
        // Start with empty tickets collection
        $tickets = collect();

        // Check if we have ticket IDs in the session (from newly created tickets)
        if (session()->has('created_tickets')) {
            $ticketIds = collect(session('created_tickets'))->pluck('id')->toArray();
            if (!empty($ticketIds)) {
        ...
        // Check if we have a viewed ticket ID, add it to the tickets if not already included
        if (session()->has('viewed_ticket_id')) {
            $viewedTicketId = session('viewed_ticket_id');

            // If this ticket isn't already in our collection, add it
            if (!$tickets->contains('id', $viewedTicketId)) {
                $viewedTicket = Ticket::where('id', $viewedTicketId)
                    ->where('user_id', Auth::id())
                    ->with(['match', 'section'])
                    ->first();

                if ($viewedTicket) {
                    $tickets->push($viewedTicket);
                }
            }
        }

        // If there's a search query, use that instead
        if ($request->has('ticket_number_search') && !empty($request->ticket_number_search)) {
            $tickets = Ticket::where('user_id', Auth::id())
                ->where('ticket_number', 'like', '%' . $request->ticket_number_search . '%')
                ->with(['match', 'section'])
                ->latest()
                ->get();
        }

        // Track highlighted ticket (most recently viewed)
        $recentlyViewedId = session('viewed_ticket_id');

        return view('tickets.index', compact('tickets', 'recentlyViewedId'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Store created tickets in the session using session()->put instead of with()
        session()->put('created_tickets', $tickets);
    ...
    public function purchase(Request $request, FootballMatch $match)
    {
        ...
        // Store created tickets in the session using session()->put
        session()->put('created_tickets', $tickets);
    ...
    {
        // Remove ticket sessions
        session()->forget('created_tickets');
        session()->forget('viewed_ticket_id');

        return redirect()->route('my-tickets')
            ->with('success', 'Your ticket list has been cleared.');
    }
}
